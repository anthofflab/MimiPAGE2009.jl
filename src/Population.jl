...
function addpopulation(model::Model)
    ...
    populationcomp[:popgrw_populationgrowth]=readpagedata(model,joinpath(dirname(@__FILE__), "..", "data", "popgrw_populationgrowth.csv"))
    populationcomp[:pop0_initpopulation]=readpagedata(model,joinpath(dirname(@__FILE__), "..", "data", "pop0_initpopulation.csv"))
