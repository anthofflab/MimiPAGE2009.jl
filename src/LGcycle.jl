...
if t==1
    ...
    v.nte_natLGemissions[t]=p.stim_emissionfeedback*p.rtl_g_0_realizedtemp
    ...
    re_remainLGbase=v.exc_excessconcLG*p.den_LGdensity
    v.re_remainLG[t]=re_remainLGbase*exp(-(p.y_year[t]-p.y_year_0)/p.res_LGatmlifetime)+
    ...
    v.nte_natLGemissions[t]=p.stim_emissionfeedback*p.rtl_g_landtemperature[t-1]
    #eq.6 from Hope (2006) - emissions to atmosphere depend on the sum of natural and anthropogenic emissions
    v.tea_LGemissionstoatm[t]=(p.e_globalLGemissions[t]+v.nte_natLGemissions[t])*p.air_LGfractioninatm/100
...
#eq.11 from Hope(2006) - LG concentration
    v.c_LGconcentration[t]=p.pic_preindustconcLG+v.exc_excessconcLG*v.re_remainLG[t]/v.re_remainLG[1]
