...
function run_timestep(s::Tolerability, t::Int64)
    ...
    for r in d.region
        ...
        if t==1
            v.atl_adjustedtolerableleveloftemprise[t,r] = min[v.atp_adjustedtolerableplateau[t,r],
                p.atl_0_adjustedtolerableplateau[r] +
                v.atr_adjustedtolerablerate[r]*(p.y_year[t]-p.y_year[t-1])]
        else
            v.atl_adjustedtolerableleveloftemprise[t,r] = min[v.atp_adjustedtolerableplateau[t,r],
                v.atl_adjustedtolerableleveloftemprise[t-1,r] +
                v.atr_adjustedtolerablerate[r]*(p.y_year[t]-p.y_year[t-1])]
        end
...
function addtolerabilitymarket(model::Model)
    ...
    tolerabilitymarketcomp[:slope_increaseintolerableratefromadaptation] = readfile("../data/market_slope.csv") #per decade!
...
function addtolerabilitynonmarket(model::Model)
    ...
    tolerabilitynonmarketcomp[:slope_increaseintolerableratefromadaptation] = readfile("../data/nonmarket_slope.csv") #per decade!
    tolerabilitynonmarketcomp[:plat_increaseintolerableplateaufromadaptation] = readfile("../data/nonmarket_plateau.csv")
