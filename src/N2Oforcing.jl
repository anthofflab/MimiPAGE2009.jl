...
function run_timestep(s::n2oforcing, t::Int64)
    ...
    if t==1
        ...
        v.over_baseoverlap=0.47*log(1+2.01e-5*(p.baseN2Oconc*p.baseCH4conc)^0.75+5.31e-15*p.baseCH4conc*(p.baseCH4conc*p.baseN2Oconc)^1.52)
    end

    over=0.47*log(1+2.01e-5*(p.baseCH4conc*p.c_N2Oconcentration[t])^0.75+5.31e-15*p.baseCH4conc*(p.baseCH4conc*p.c_N2Oconcentration[t])^1.52)
    v.f_N2Oforcing[t]=p.f0_N2Obaseforcing+p.fslope_N2Oforcingslope*(sqrt(p.c_N2Oconcentration)-sqrt(p.c0_baseN2Oconc))+over-v.over_baseoverlap
end
