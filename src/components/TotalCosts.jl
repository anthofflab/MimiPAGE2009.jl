@defcomp TotalCosts begin
    ...
    period_span = Parameter(index = [time], unit="year")
    ...
    total_damages                   = Variable(index = [time, region], unit = "\$million")  # Undiscounted total damages per period
    ...
    total_abatement                 = Variable(index = [time, region], unit = "\$million")  # Undiscounted total abatement per period 
    ...
    total_costs                     = Variable(index = [time, region], unit = "\$million")  # Undiscounted total costs per period                  
    ...
    function run_timestep(p, v, d, t)
        ...
        v.total_damages_peryear[t, :] = v.total_damages_percap[t, :] .* p.population[t, :]
        v.total_damages[t, :] = v.total_damages_peryear * p.period_length[t]

        # Abatement
        v.total_abatement_percap_peryear[t, :] = p.abatement_costs_percap[t, :]
        v.total_abatement_peryear[t, :] = v.total_abatement_percap_peryear[t, :] .* p.population[t, :]
        v.total_abatement[t, :] = v.total_abatement_peryear[t, :] .* p.period_length[t, :]
        ...
        v.total_costs[t, :] = v.total_costs_peryear[t, :] .* p.period_length[t, :]
    end
end