@defcomp AbatementCostParameters begin
    ...
    q0propmult_cutbacksatnegativecostinfinalyear = Parameter(unit="none", default=0.733333333333333334)
    qmax_minus_q0propmult_maxcutbacksatpositivecostinfinalyear = Parameter(unit="none", default=1.2666666666666666)
    c0mult_mostnegativecostinfinalyear = Parameter(unit="none", default=0.8333333333333334)
    curve_below_curvatureofMACcurvebelowzerocost = Parameter(unit="none", default=0.5)
    curve_above_curvatureofMACcurveabovezerocost = Parameter(unit="none", default=0.4)
    cross_experiencecrossoverratio = Parameter(unit="none", default=0.2)
    learn_learningrate = Parameter(unit="none", default=0.2)
    automult_autonomoustechchange = Parameter(unit="none", default=0.65)
...
function addabatementcostparameters(model::Model, class::Symbol, policy::String="policy-a")
    ...
    if class == :CO2
        ...
        update_param!(model, componentname, :q0propinit_CutbacksinNegativeCostinFocusRegioninBaseYear, 20.)
        update_param!(model, componentname, :c0init_MostNegativeCostCutbackinBaseYear, -233.333333333333)
    ...
    elseif class == :CH4
        ...
        update_param!(model, componentname, :q0propinit_CutbacksinNegativeCostinFocusRegioninBaseYear, 10.)
        update_param!(model, componentname, :c0init_MostNegativeCostCutbackinBaseYear, -4333.3333333333333)
    ...
    elseif class == :N2O
        ...
        update_param!(model, componentname, :q0propinit_CutbacksinNegativeCostinFocusRegioninBaseYear, 10.)
        update_param!(model, componentname, :c0init_MostNegativeCostCutbackinBaseYear, -7333.333333333333)
        update_param!(model, componentname, :qmaxminusq0propinit_MaxCutbackCostatPositiveCostinBaseYear, 51.66666666666666664)
